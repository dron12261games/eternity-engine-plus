...
* T\_YELLOW - index 160 to 167
* T\_BLUE - index 196 to 207
* T\_NAVY - index 240 to 247
* T\_MAGENTA - index 250 to 255

The default tables define the **sbarback**, **interback**, and the **table** fields to their **doom1.9** equivalents (with the extra translations using the T\_GREEN background and translated with the defined table at render time), with the order of entries defined above corresponding to both player numbers and the order of lumps resolved for multiplayer purposes.
...
Any thing that has the MF\_TRANSLATION field set in its mobjinfo\_t flags and does not have its **Translation** field set must resolve its translation from the **playertranslations** table.
