# ID24 Mapping Additions formal specification 0.99.1

ID24 has implemented a few new features and line specials for use by mappers. This document details this new functionality.
...
* All Boom map features (including generalised line and sector specials)
...
The following values/ranges are reserved for implementation-defined line specials (or in other words, port-specific lines):
...
All other ranges not covered by a prior specification or by these ranges are considered community ranges and should be used for new specifications and other agreed-upon uses.
...
The “play once” variants of the specials use the **doom1.9** functionality for play once, ie once the music track finishes make no attempt to loop the track nor switch tracks to another.
...
| 2056 | Always | Offset then rotate target floor and ceiilng texture by line direction and angle. |
