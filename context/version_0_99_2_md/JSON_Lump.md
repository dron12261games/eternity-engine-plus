# JSON Lump formal specification version 0.9.2
...
| --- | --- | --- |
| **type** | string | The name of the type of data this lump contains. Used for verification purposes at engine runtime; used by editors to determine how to handle the lump.  This value must be lowercase; have one character minimum; and consist of letters, numbers, underscores, and dashes only. The regex string `^[a-z0-9\_-]+$` should thus result in an exact match with the provided value. |
...
In order to facilitate comments from a tooling perspective, each subobject of the **data** field is allowed to have a **comment** field of type string. Note that ISO/IEC 21778:2017 does not support C/C++ style comments, which complicates how a document is parsed if a tool were to support such comments.
...
```c
jsonlumpresult M_ParseJSONLump( lumpindex_t lumpindex, const char* lumptype, const JSONLumpVersion& maxversion, const JSONLumpFunc& parsefunc );
```
